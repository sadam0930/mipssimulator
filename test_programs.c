...
{
  int code_addr = 0;
  uint32_t *code_segment = (uint32_t *) &(memory[CODE_STARTING_ADDRESS]);
  int label1;
...
void instruction_test()
{
  ...
  uint32_t *x_ptr = (uint32_t *) &(memory[data_addr]);
  data_addr += 4;  //made room for x

  *x_ptr = 0x110111; // x = 1114385

  //  code_segment[code_addr++] = ORI_INSTRUCTION(T0,ZERO, DATA_STARTING_ADDRESS);
  ...
  //Testing SLL, by shifting A0 left by 2. Since A0 contains 345, the result should be 1380
  ...
  //Testing SUB, and SRA. Put -25 into A0 and shift right by 2. The result should be -6
  ...
  //T5 holds -1, T6 holds 200, so should get -201 as a result (when printed as a signed number, of course).
  ...
  //Testing DIV_INSTRUCTION(__rs,__rt) 
...
void syscall_test()
{
  uint32_t code_addr = 0;
  uint32_t *code_segment = (uint32_t *) &(memory[CODE_STARTING_ADDRESS]);
  uint32_t data_addr = DATA_STARTING_ADDRESS; //careful, this may be more than 16 bits.
  uint32_t i;
    
  //print_string test
  uint8_t prompt[] = "Enter Number >";
  uint32_t prompt_label = data_addr;
  ...
  memory[data_addr++] = 0;
  //Another prompt
  uint8_t prompt_str[] = "Enter String >";
  uint32_t prompt_str_label = data_addr;
  //copy string into memory at the beginning of the data section
...
#define STRING_BUF_SIZE 80
  uint32_t input_string_addr = data_addr; //careful, may need more than 16 bits
